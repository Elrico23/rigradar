...
class CompiledMap {
  constructor({ meta, roads, search, router, signs, cityAreas, ferries, facilities }) {
    this.meta = meta;
...
async function loadCompiledMap(dataDir) {
  const [meta, roadsBuf, search, signs, cityAreas, ferries, facilities, nodes] = await Promise.all([
  ...
  return new CompiledMap({ meta, roads, search, router, signs, cityAreas, ferries, facilities });
}
