...
const { decodeAreas } = require('./areaformat');
...
class CompiledMap {
  constructor({ meta, roads, search, router, signs, cityAreas, ferries, facilities, areas }) {
    this.meta = meta;
    ...
    this.facilities = facilities || []; // array of {x, z, kind} — gas, rest, repair, etc.
    this.areas = areas || { count: 0, areas: [] }; // decoded lot/building footprint polygons
  }
  ...
  /** Facilities (gas, rest stops, repair shops, etc.) within a viewport —
   * same "only useful at close zoom" reasoning as signsInView. */
  facilitiesInView(minX, maxX, minZ, maxZ) {
    const out = [];
  ...
  /** Lot/building footprint polygons touching a viewport — same
   * any-vertex-in-box test as ferriesInView. These are small (tens of
   * metres across), so unlike a city footprint there's no case worth
   * handling where a polygon spans the whole box with no vertex inside it. */
  areasInView(minX, maxX, minZ, maxZ, pad = 100) {
    const lo_x = minX - pad, hi_x = maxX + pad;
    const lo_z = minZ - pad, hi_z = maxZ + pad;
    const out = [];
    for (const a of this.areas.areas) {
      if (polylineHitsBox(a.pts, lo_x, hi_x, lo_z, hi_z)) out.push({ color: a.color, pts: Array.from(a.pts) });
    }
    return out;
  }
...
async function readOptionalBin(dataDir, filename) {
  const file = path.join(dataDir, filename);
  return fs.existsSync(file) ? fsp.readFile(file) : null;
}

async function loadCompiledMap(dataDir) {
  const [meta, roadsBuf, search, signs, cityAreas, ferries, facilities, nodes, areasBuf] = await Promise.all([
    ...
    readOptionalJson(dataDir, 'facilities.json'),
    readOptionalJson(dataDir, 'graph.json'),
    readOptionalBin(dataDir, 'areas.bin'),
  ]);
  const roads = decodeRoads(roadsBuf);
  const router = nodes.length ? new Router(nodes) : null;
  const areas = areasBuf ? decodeAreas(areasBuf) : null;

  return new CompiledMap({ meta, roads, search, router, signs, cityAreas, ferries, facilities, areas });
}
